define(function(require, exports, module) {
	...
	function be_star(id){
		...
		id.each(function(){
			...
			$list_i.slice(_star,$list_i.length).addClass('star-02');
		});
	}
	be_star($ct_star);
	...
	$ct_pj.on('click',function(){
		var $ps,
			...
			$dt_2     =$('.dt-02',$release),
			$_span    =$('span',$dt_2),
			$_fn_star =$('.fn-star:not([data-name])',$release);
	...
	function v_star($this){
		if(!$this.length){
			return true;
		}
		var $fn_star=$('.fn-star',$this),
			_bol=false,
			_list=[];
		$fn_star.each(function(){
			var $this=$(this);
				_list.push(
					{
						name:$this.data("name"),
	...
	// $ct_st.on('shop-v','.st-shop',function(){
